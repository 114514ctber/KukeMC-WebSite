        ...
        {isOpen && (
          ...
          >
            ...
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="relative w-full max-w-6xl h-[85vh] bg-[#0f0f11] rounded-[2rem] shadow-2xl border border-white/10 overflow-hidden flex flex-col md:flex-row z-10"
            >
              {/* Decorative Background Elements */}
              <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
                <div className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 bg-red-500/10 rounded-full blur-[120px]" />
                <div className="absolute top-1/2 -right-1/4 w-1/2 h-1/2 bg-purple-500/10 rounded-full blur-[120px]" />
                <div className="absolute bottom-0 left-1/3 w-1/3 h-1/3 bg-orange-500/10 rounded-full blur-[100px]" />
              </div>

              {/* Close Button (Mobile) */}
              <button
                onClick={onClose}
                className="absolute top-4 right-4 z-50 p-2 rounded-full bg-black/20 text-white/70 hover:bg-white/10 hover:text-white md:hidden backdrop-blur-sm"
              >
                <X className="w-5 h-5" />
              </button>

              {/* Left Column: Player Model & Identity */}
              <div className="relative w-full md:w-[320px] lg:w-[380px] bg-gradient-to-b from-[#1a1a1e] to-[#0f0f11] flex flex-col items-center border-b md:border-b-0 md:border-r border-white/5 shrink-0 z-10">
                 {/* Player Identity Header */}
                 <div className="w-full p-6 pb-0 flex flex-col items-center text-center">
                    <div className="mb-4 relative group">
                       <div className="absolute inset-0 bg-gradient-to-br from-red-500 to-orange-500 rounded-full blur opacity-40 group-hover:opacity-60 transition-opacity duration-500" />
                       <div className="relative w-28 h-28 rounded-2xl overflow-hidden border-2 border-white/10 bg-[#151518] shadow-xl">
                          <img 
                    ...
                    <h2 className="text-2xl font-black text-white tracking-tight mb-2">{stats.name}</h2>
                    <div className="flex items-center justify-center flex-wrap gap-2 text-white/50 text-sm">
                      <div className="px-3 py-1 bg-[#151518] border border-white/10 rounded-full shadow-sm flex items-center">
                 ...
                 <div className="flex-1 w-full flex items-center justify-center relative min-h-[300px] md:min-h-0">
                    ...
                    <motion.img 
                      ...
                      alt="Player Body"
                      className="relative z-10 h-[90%] object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] transform hover:scale-105 transition-transform duration-500"
                    />
                 </div>
                 ...
                 <div className="w-full p-6 pt-0">
                    <div className="p-4 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm">
                       <div className="flex justify-between items-center mb-2">
                          <span className="text-xs font-medium text-white/40 uppercase tracking-wider">最爱职业</span>
                       ...
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <div className="font-bold text-white">{stats.favorite_kit || '暂无'}</div>
                             <div className="text-xs text-white/40">常用首选</div>
                          </div>
                       </div>
                    </div>
                 </div>
              ...
              <div className="flex-1 flex flex-col min-w-0 bg-[#0f0f11]/50 relative z-10">
                {/* Header Actions */}
                <div className="flex items-center justify-between p-6 border-b border-white/5">
                   ...
                   <button
                    onClick={onClose}
                    className="hidden md:flex p-2 rounded-full hover:bg-white/10 transition-colors text-white/50 hover:text-white"
                   >
                    <X className="w-6 h-6" />
                   </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                  ...
                  >
                    ...
                    <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                       <div className="lg:col-span-2 p-5 rounded-2xl bg-[#151518] border border-white/5 flex items-center justify-between relative overflow-hidden group">
                          ...
                          <div className="flex items-center gap-4 relative z-10">
                             ...
                             <div>
                                <p className="text-sm text-white/40 uppercase tracking-wider font-medium">总经验值</p>
                                <div className="flex items-baseline gap-2">
                                   <AnimatedCounter value={exp} className="text-2xl font-bold text-white" />
                                   <span className="text-xs text-white/30">EXP</span>
                                </div>
                             </div>
                          </div>
                          {/* Progress bar visual */}
                       ...
                       <div className="p-5 rounded-2xl bg-[#151518] border border-white/5 flex items-center gap-4 relative overflow-hidden group">
                          ...
                          <div className="p-3 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/10 relative z-10">
                             <Sparkles className="w-6 h-6" />
                          </div>
                          <div className="relative z-10">
                             <p className="text-sm text-white/40 uppercase tracking-wider font-medium">近期活跃</p>
                             <p className="text-white font-medium">状态良好</p>
                          </div>
                       </div>
                    </motion.div>
                    ...
                    <motion.div variants={itemVariants}>
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-xs text-white/30">按击杀数排序</span>
                      </div>
                      ...
                      {stats.kits_stats && stats.kits_stats.length > 0 ? (
                        ...
                        <div className="p-12 text-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02]">
                          ...
                          <p className="text-white/40">暂无职业使用数据</p>
                        </div>
                      )}
                    </motion.div>
                  </motion.div>
...
function StatCard({ label, value, icon: Icon, color, gradient, borderColor, isFloat = false }: any) {
  return (
    <motion.div 
      ...
      className={clsx(
        "p-5 rounded-2xl bg-[#151518] border relative overflow-hidden group transition-all duration-300 hover:-translate-y-1",
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div>
          <div className="text-2xl font-bold text-white tracking-tight flex items-baseline gap-1">
             ...
             {isFloat && <span className="text-sm text-white/40 font-normal">ratio</span>}
          </div>
          <div className="text-xs font-medium text-white/40 uppercase tracking-wider mt-1">{label}</div>
        </div>
      </div>
    </motion.div>
  );
    ...
    >
       <div className="flex items-center gap-4 relative z-10">
         ...
         <div className="flex flex-col min-w-0">
           <span className={clsx("font-bold text-lg leading-tight truncate", isTop3 ? "text-white" : "text-white/70")}>
             {kit.kit_name}
           </span>
           {index === 0 && <span className="text-[10px] text-yellow-500 font-bold uppercase tracking-wider">Champion</span>}
           {index === 1 && <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Runner-up</span>}
           {index === 2 && <span className="text-[10px] text-orange-400 font-bold uppercase tracking-wider">Third Place</span>}
         </div>
       </div>
       
       <div className="flex flex-col items-end relative z-10 pl-2">
          <span className={clsx("text-xl font-black tracking-tight", isTop3 ? "text-white" : "text-white/60")}>
            {kit.kills.toLocaleString()}
          </span>
          <span className="text-[10px] text-white/20 font-bold uppercase tracking-wider">Kills</span>
       </div>
